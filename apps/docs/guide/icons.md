...
为保持图标库视觉一致，新增或替换 SVG 时请遵循以下规范：

| 维度 | 规范 | 说明 |
| --- | --- | --- |
| 画布 | 24×24 viewBox | 统一画布尺寸，保证不同图标在相同 `size` 下视觉权重一致 |
| 描边 | 1.5px ~ 2px | 线性图标统一描边粗细，避免部分图标过细或过粗 |
| 端点 | 圆角端点（round） | 保持整体圆润风格，与组件圆角体系呼应 |
| 转角 | 圆角连接（round） | 拐角处不过于尖锐，符合现代 UI 风格 |
| 着色 | `currentColor` 唯一 | 禁止写死 `fill` / `stroke` 色值，确保主题/暗色切换正常 |
| 命名 | `目录/文件名` 层级 | 与 `assets/icons-v2/<目录>/<文件>.svg` 一一对应，语义清晰 |

> 已有图标若存在风格不一致（描边粗细、端点、画布），建议在设计侧统一走一遍规范后再批量替换。
