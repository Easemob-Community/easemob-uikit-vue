...
| `packages/uikit/src/assets/icons-next/**` | ~~设计师交付的**新版候选素材库**，用于补充/替换 `assets/icons`。~~ **已清理删除**。 | ❌ 不应被直接引用 | ❌ 已于 2026-08-07 删除，避免与运行时目录并存造成误解 |
...
- **2026-08-07**：根据业务确认，`packages/uikit/src/assets/icons-next/` 已整目录删除。后续设计师新交付的图标素材统一放到仓库外（如设计稿/蓝湖/Figma），由负责工程师按本策略评估后，再复制/替换到 `assets/icons`。
