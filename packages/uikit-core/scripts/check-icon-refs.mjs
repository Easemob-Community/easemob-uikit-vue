 ...
 * check-icon-refs.mjs
 *
 * 构建期校验：扫描指定包 src 下所有 .vue / .ts（含 *.story.vue，它们也真实渲染），
 * 提取 EmIcon 的 name 引用（模板 name="分类/图标名"、:name="'...'"）以及 TS 中的
 * icon: '...' 等图标名字面量，与 icons 目录下实际存在的 SVG 文件比对。
 ...
 *
 * 参数化（工具链防复制，P1 Step 4 迁入 core，uikit-im 复用同一脚本）：
 *   node check-icon-refs.mjs [scanPkgRoot] [iconsDir]
 *   - scanPkgRoot：待扫描包根（默认脚本所属包，即 uikit-core）
 *   - iconsDir：图标库目录（默认 <scanPkgRoot>/src/assets/icons）
 * 执行方法：
 *   core：cd packages/uikit-core && pnpm run icons:check
 *   uikit-im：cd packages/uikit-im && pnpm run icons:check
 *   （uikit-im 侧扫描自身 src，图标库指向 ../uikit-core/src/assets/icons）
 */
...
const iconsDir = resolve(process.argv[3] ?? join(srcDir, 'assets/icons'))
...
if (categories.length === 0) {
  console.error(`未找到图标分类目录: ${iconsDir}`)
  process.exit(1)
}
