...
/** 导出日志文件名的前缀（场景包可自定义，如 'easemob-uikit-im'；默认 'easemob-uikit'） */
let _logFilePrefix = 'easemob-uikit'

/**
 * 设置导出日志文件名的前缀（`${prefix}-logs-<时间戳>.log`）。
 * 场景包（uikit-im / uikit-chatroom）在入口 side-effect 中调用，保持各自下载文件名语义；
 * 该设置独立于 configureLogPersistence，后续配置变更不会覆盖它。
 */
...
export async function exportPersistedLogs(): Promise<number> {
  ...
  a.href = url
  a.download = `${_logFilePrefix}-logs-${stamp}.log`
  a.click()
