...
import type { CombineMessageBody, ConversationTypeValue, UiMessage } from '@easemob/uikit-core'
import { customEventPreviewMap } from '../../utils/resolve-last-message-text'

const adapterLog = createLogger('UIKit:MessageAdapter')

/**
 * 归一化用户标识（跨设备/资源后缀场景下的身份比较）。
 * P2 review 上提至 core（uikit-im / uikit-chatroom 共用），此处 re-export
 * 保持既有导入路径兼容（本包 6 处使用点无需改动）。
 */
export { normalizeUserId } from '@easemob/uikit-core'
