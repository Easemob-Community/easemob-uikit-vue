...
import { usePullRefresh } from '@easemob/uikit-core'
...
import { EmModal as Modal } from '@easemob/uikit-core'
import type { FileMessageBody, LocationMessageBody, TextMessageBody, UiMessage } from '@easemob/uikit-core'
...
export interface MessageListProps {
  /** 消息列表配置（仅消费 config.messageList 子树：布局 / 头像 / 时间戳 / 气泡形状 / 消息状态等） */
  config?: ChatConfig
}

export interface MessageListEmits {
  /** 点击自己撤回消息上的"重新编辑"按钮时触发，参数为目标消息 / Emitted when the "re-edit" button of a self-recalled message is clicked, with the target message */
  ...
  /** 点击消息操作菜单中的"编辑"时触发（仅文本且未撤回的消息），参数为目标消息 / Emitted when "Edit" in the message action menu is clicked (text, non-recalled only), with the target message */
  (e: 'edit', message: UiMessage): void
  /** 点击消息操作菜单中的"转发"时触发，参数为待转发的消息数组（当前为单条）/ Emitted when "Forward" in the message action menu is clicked, with the messages to forward (currently a single item) */
  ...
  /** 右键消息头像选择"查看资料"时触发，参数为消息发送者 ID / Emitted when "View profile" is chosen from the avatar context menu, with the sender ID */
...
async function onGroupReadClick(msgId: string, groupId: string) {
  ...
  try {
    ...
    if (readUsers.length === 0) {
      ...
      stores.message.setGroupMessageReadUsers(groupId, msgId, readUsers)
      // 用服务端权威 count 更新消息已读数，补齐离线消息/多端同步等场景下气泡数字缺失
      if (result && result.count > 0) {
        stores.message.updateMessageById(msgId, { groupReadCount: result.count })
      }
    }
...
<template>
  <div class="message-list">
    ...
    >
      ...
      <Icon name="arrow/down" :size="12" class="message-list__new-tip-arrow" />
    </div>
