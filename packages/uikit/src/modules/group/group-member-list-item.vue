...
function getMoreActions(member: UiGroupMember): MoreAction[] {
  ...
  if (props.showAdminAction && canSetAdmin(member))
    actions.push({ key: 'setAdmin', icon: 'actions/shield_person', label: t('group.memberList.setAdmin', '设管理员') })
  if (props.showAdminAction && canRemoveAdmin(member))
    actions.push({ key: 'removeAdmin', icon: 'actions/shield_xmark', label: t('group.memberList.removeAdmin', '取消管理员') })
