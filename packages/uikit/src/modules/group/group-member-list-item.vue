...
function getMoreActions(member: UiGroupMember): MoreAction[] {
  ...
  if (props.showAdminAction && canRemoveAdmin(member))
    actions.push({ key: 'removeAdmin', icon: 'group-manager/icon/stroked/shield/person', label: t('group.memberList.removeAdmin', '取消管理员') })
  if (canTransferOwner(member))
    actions.push({ key: 'transferOwner', icon: 'group-manager/icon/stroked/crown', label: t('group.memberList.transferOwner', '转让群主'), danger: true })
