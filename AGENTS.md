...
- **版本号同步**：`packages/uikit-im/package.json` 的 `version` 必须与根 `CHANGELOG.md` 最新版本段一致；发版/改版本号后提交前跑 `pnpm changelog:check`（唯一版本数据源是根 CHANGELOG，文档站 changelog 页通过 tabs 引入按包自动拆分的生成文件，禁止手写版本段）。
...
| Skill                              | 适用场景                                                                                              | 触发词示例                                                                                                                           | 文件                                                                                                               |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| `uikit-package-boundary`           | 三包归属判定：新功能（store/composable/组件/utils/domain/event/常量/文案）放 core 还是场景包；core 隔离规则与门禁                                                         | `新功能放哪`、`进 core`、`core 还是 im`、`包边界`、`加 store/组件 放哪个包`、`core 隔离`                                            | [.agent/skills/uikit-package-boundary/SKILL.md](.agent/skills/uikit-package-boundary/SKILL.md)                     |
...
| `uikit-uiux-acceptance`            | UI/UX 验收规范：无障碍 / 热区 / 间距 / Type Scale / Do/Don't / 设计原则                               | `UIUX 验收`、`无障碍`、`focus-visible`、`44pt`、`Do/Don't`、`Design Tokens`、`对比度`                                                | [.agent/skills/uikit-uiux-acceptance/SKILL.md](.agent/skills/uikit-uiux-acceptance/SKILL.md)                       |
...
| `uikit-chatroom-design`            | 聊天室场景包设计约束：组件分层、通用壳子（InteractiveCard/OverlayManager/DanmakuStream）、属性 key 与弹幕类型契约 | `chatroom 设计`、`聊天室设计约束`、`live 组件分层`、`InteractiveCard`、`OverlayManager`、`商品卡/红包/优惠券`、`直播间 overlay`       | [.agent/skills/uikit-chatroom-design/SKILL.md](.agent/skills/uikit-chatroom-design/SKILL.md)                       |
