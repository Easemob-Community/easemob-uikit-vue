...
- **进度（2026-08-15，P5 文档与集成完成，`@easemob/uikit-chatroom` 升 1.0.0）**：① **`gen:api` 参数化**（`PACKAGES` 配置表驱动 uikit-im + chatroom，输出隔离 `.vitepress/gen/chatroom/`，uikit-im 根目录零漂移）；props 查找新增同目录 `types.ts` 声明兜底（`ChatroomContainerProps/Emits`）与内联 `defineProps<{...}>()` 兜底；**顺带修正 P1 抽核遗留**：原子组件根目录迁移到 `@easemob/uikit-core`（此前指向 uikit-im 必 ENOENT）、插槽提取增强（`name` 属性不在 `<slot` 首位的多行写法漏提取，容器 19 插槽此前仅落 10 个）；② **docs 聊天室章节正式化**：chatroom-container / live-danmaku / 直播组件集（新页）/ pc-mode-components 全接入自动 API 表格，弹幕流页新增运行 demo 块（纯 UI 模拟数据，docs 补 `@easemob/uikit-chatroom` workspace 依赖）；③ **聊天室集成 skill**（`integrations/skills/chatroom`：SKILL.md + quickstart/provider/components/danmaku/pc-mode/gotchas + api 自动同步；`gen-skill.mjs` 参数化双目标，一次同步 39 个 API）；④ **MCP 数据纳入聊天室**（`sync-docs.mjs` 增 chatroom guide/api/version，manifest 增 `chatroom` 段，`search_docs` 覆盖两包、`get_component_api`/`list_components` 兼容聊天室）。验收：docs build 全绿（19.25s）、MCP vue-tsc/build 通过、`changelog:check` 通过（1.0.0 版本段 + package.json 同步）。**至此 P0→P5 全阶段完成，D97 关项**。
