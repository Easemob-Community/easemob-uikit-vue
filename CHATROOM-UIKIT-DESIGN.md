...
- **P5 文档与集成**（**已完成，2026-08-15，版本 1.0.0**）：docs 站聊天室章节（gen:api、demo 块、sidebar）、聊天室集成 skill（`integrations/skills`）、MCP 数据更新（`packages/mcp/scripts/sync-docs.mjs`）。（**docs 双 UIKit 架构骨架已提前落地**：`apps/docs` 复用 VitePress locales 承载 `/` 与 `/chatroom/` 两套并列文档树，顶部标题旁 `UiKitDocsSwitcher` 切换。**P5 收尾实施**：`gen:api` 参数化（`PACKAGES` 配置表 + 输出隔离 `.vitepress/gen/chatroom/` + types.ts/内联 defineProps props 兜底 + 插槽提取增强，顺带修正 P1 抽核遗留的原子组件根目录）；组件页全部接入自动生成 API 表格（chatroom-container / live-danmaku / 直播组件集新页 / pc-mode-components）+ 弹幕流运行 demo 块；聊天室集成 skill（`integrations/skills/chatroom`，`gen-skill.mjs` 参数化双目标）；MCP 数据纳入聊天室（guide + API + manifest `chatroom` 段，`search_docs` 覆盖两包）。**验收：docs build 全绿、`changelog:check` 通过、门禁全绿。**）
